...
대상 종목: {stock_name} ({stock_sector})
  ※ stock_sector는 "테마명(등락률)" 형식으로 제공됩니다 (예: 반도체(+1.5%), AI칩(+2.3%))

1. 시장 및 테마 환경 (가장 중요):
   - 시장 지수(Market): {market_index_status} (예: 코스피 -1% 하락장 등)
   - 오늘 주도 테마: {current_hot_themes}
   - 종목 소속 테마: {stock_sector}
   - **테마 분석 포인트:** 이 종목이 속한 테마의 등락률이 높다면(+2% 이상) 강세 섹터에 속한 것이므로 긍정적 신호입니다.
     반대로 테마 등락률이 마이너스라면 해당 섹터 전체가 약세이므로 주의가 필요합니다.
